...
import { getGoogleAccessToken, getGoogleUserInfo } from '@/lib/auth/oauth';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const existingUser = await query(
      `SELECT id, email, name, role, status FROM users WHERE email = $1 AND auth_provider = 'google'`,
    ...
    if (existingUser.rows.length > 0) {
      ...
      if (pendingCheck.rows.length === 0) {
        ...
        await query(
          `INSERT INTO pending_users (email, name, auth_provider, provider_user_id, avatar_url, status)
           VALUES ($1, $2, 'google', $3, $4, 'pending')`,
          [userInfo.email.toLowerCase(), userInfo.name, userInfo.id, userInfo.picture]
        );
        ...
        await query(
          ...
          [JSON.stringify({ email: userInfo.email, provider: 'google' })]
        );
      }
